...
function Assert-ExecutorShape([object]$Executor, [switch]$RequireEnvelope) {
    if ($RequireEnvelope -and
        ($Executor.schema -ne "machine-utilities.executor" -or $Executor.schema_version -ne 1)) {
        throw "Invalid executor requirement envelope"
    }
    if ($Executor.plugin -ne "machine-utilities" -or
...
function Assert-Executor([object]$Required, [object]$Reported, [string]$Root = $PluginRoot) {
    ...
    if ($Manifest.schema -ne "machine-utilities.integrity" -or $Manifest.schema_version -ne 1 -or
...
function Get-InstalledExecutor([string]$Root = $PluginRoot) {
    ...
    $Manifest = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
    if ($Manifest.schema -ne "machine-utilities.integrity" -or $Manifest.schema_version -ne 1 -or
        $Manifest.plugin -ne "machine-utilities" -or
...
function Assert-Plan([object]$Plan, [string]$WorkerConfigDigest) {
    if ($Plan.schema -ne "machine-utilities.plan" -or $Plan.schema_version -ne 2 -or
...
if ($SelfTest) {
    ...
    try {
        ...
        $FixtureManifest = [ordered]@{
            schema = "machine-utilities.integrity"
            schema_version = 1
...
if ($null -ne $Failure) {
    [Console]::Error.WriteLine("machine-utilities: Windows apply failed at $($Failure.stage)")
    exit 70
}

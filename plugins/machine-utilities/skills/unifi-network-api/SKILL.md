...
```sh
scp root@unifi:/usr/lib/unifi/webapps/ROOT/api-docs/integration.json ~/.codex/skills/unifi-network-api/references/current-openapi.json
jq -r '[.info.title,.info.version,.openapi,.servers[0].url] | @tsv' ~/.codex/skills/unifi-network-api/references/current-openapi.json
```
